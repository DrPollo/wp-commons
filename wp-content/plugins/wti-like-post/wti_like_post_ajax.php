			...
			if ( function_exists( 'bp_activity_add' ) ){
				if ( version_compare( BP_VERSION, '1.3', '>' ) ){
					...
					bp_activity_add( array(
						'user_id' => $user_id,
						//'item_id' => $postlink,
						'action' => sprintf( __( '%s like the Idea: %s', 'wti' ), $userlink, '<a href="'.get_permalink($post_id).'">'.get_the_title($post_id).'</a>' ),
						//'content' => '<a href="'.get_permalink($post_id).'" class="add-to-guide-tracce">'.get_the_post_thumbnail($post_id).'</a>',
						'primary_link' => '<a href="'.get_permalink($post_id).'">'.get_the_title($post_id).'</a>',
						'component' => 'profile',
						'type' => 'like_idea',
					) );
			}}
